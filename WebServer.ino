...
String read_totalMilliLitres() {
  // Sensor readings may also be up to 2 seconds 'old' (its a very slow sensor)
  float f = dht.read_totalMilliLitres();
  if (isnan(f)) {
    Serial.println("Failed to read from DHT sensor!");
    return "--";
  }
...
<body>
  ...
  <p>
    ...
    <span id="humidity">%FLOWTOTAL%</span>
    <sup class="units">&percnt;</sup>
  </p>
...
setInterval(function ( ) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("flow").innerHTML = this.responseText;
    }
  };
...
String processor(const String& var){
  ...
  else if(var == "FLOWTOTAL"){
    return read_totalMilliLitres();
  }  
  return String();
}
...
void WebServer_setup() {
 ...
 dht.begin();
  ...
  server.on("/flow", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", read_totalMilliLitres().c_str());
  });
